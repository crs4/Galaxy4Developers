...
```bash
$ mkdir planemo_tutorial
```
Cd into the tutorial directory
```bash
$ cd planemo_tutorial
```
Create a new virtual environment into the folder `.venv`
```bash
$ virtualenv .venv
```
...
From now on, any package that you install using [pip](https://pip.pypa.io/en/stable/) will be placed into the 
tutorial folder, isolated from the global Python installation.
![Virtualvenv]({{site.url}}/images/08_venv_activate.png)
